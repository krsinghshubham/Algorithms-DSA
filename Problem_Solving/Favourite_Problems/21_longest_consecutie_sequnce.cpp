...
/* time: O(nlogn),space: O(1)
    BEST : algo: 
     ...
     for each elemnet (it) of set, approach in two ways if its succerror element is not present or not, or its precessdderrecor elemnnt is present or not
     for ex:  for a x if  its x-1 is not presetn, means its smallest no of a sequence;
     then set x as current_num
     and incremnet current_length till x+1 is present in the sequence.
