...
void printWithoutRecursion(Node *head) 
{
    int size = 0;
    while (head != nullptr)
    {
...
* RECURSIVE METHOD TO REVERSE (! important too. !)
...
int main()
{
    ...
    // * ONLY ONE CAN BE EXECUTED AT A TIME BECAUSE THE LINKED LISTS WILL CHANGE.
    cout << "\nReversed Linked list after revering recursively: \n";
    Node *newHeadSecond = reverseList(head);
    printWithoutRecursion(newHeadSecond);

    // cout << "\nReversed Linked list after reversing recursively Leetcode logic: \n";
    // Node *newHeadSecond = reverseList(head);
    // printWithoutRecursion(newHeadSecond);
    return 0;
}