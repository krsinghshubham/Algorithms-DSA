...
{
public:
    void rotate(ListNode *&head) //* passing by refernce , VERY IMPORTANT
    {
        ListNode *temp = head;
...
public:
    ListNode *rotateRight(ListNode *head, int k)
    {
        ...
        // bachha list ke next to suru me dalna
        forLen = head; // not assigning to save memory space
        k = k % count; // to handle cycles
        if (k == 0)    // no rotation required wii be at same position.
            return head;
        count = count - (k + 1); // 1 to be at one place back
        while (count--)
        {
            forLen = forLen->next;        // position reached
        }                                 // forLen is at third...
        ListNode *result = forLen->next;  // store for result otherwise it will change
        ListNode *newHead = forLen->next; // bachhe list ka head
        while (newHead->next)
        ...
        forLen->next = nullptr;      // jaha se tuta tha uske next me nullptr.
