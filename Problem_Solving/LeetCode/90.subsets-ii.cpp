//* Without sorting, poor solution but it makes the problem even tougher. Brute force and Optimised both approaches.

//* Optimise: Time: 4ms ,Memory: 18mb.
//* The idea is to sort the vector so that we can directly jump the duplicates and make use of set to avoid it

class Solution
{
public:
  // first sort the nums
  // since nums is sorted so duplicate subsets will occur consecutively. so before adding
...
//* Brute Force: Time: 640ms, Memory 166mb.
class Solution
{
public:
